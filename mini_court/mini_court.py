...
class MiniCourt():
    ...
    def set_court_drawing_key_points(self):
        ...
        if len(drawing_key_points) < 20:
          print(f"Error: drawing_key_points has fewer than 20 elements. Length: {len(drawing_key_points)}")
    
        print(f"drawing_key_points: {drawing_key_points}")  # Print keypoints to check their values
    ...
    def draw_court(self, frame):
     for i in range(0, len(self.drawing_key_points), 2):
        if i + 1 >= len(self.drawing_key_points):  # Check if index i+1 exists
            print(f"Error: index {i + 1} out of range for drawing_key_points")
            continue  # Skip this iteration if index doesn't exist
        ...
        for line in self.lines:
          try:
            ...
            print(f"Error in draw_court: {e}")
            continue  # Skip this iteration in case of index error
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self, player_boxes, ball_boxes, original_court_key_points):
     ...
     for frame_num, player_bbox in enumerate(player_boxes):
        # Ensure the ball_boxes list has enough frames
        if frame_num >= len(ball_boxes):
            print(f"Error: ball_boxes does not contain data for frame {frame_num}")
            output_player_boxes.append({})
            continue  # Skip to the next frame if no ball data

        # Check if the ball_boxes entry is valid and can be unpacked
        if not ball_boxes[frame_num] or len(ball_boxes[frame_num]) < 2:
            print(f"Error: No valid ball box for frame {frame_num}")
            output_player_boxes.append({})
            continue  # Skip this frame if ball_boxes is missing or invalid
        
        ball_box = ball_boxes[frame_num][1]  # Safely unpack the ball box
        if ball_box is None:  # Ensure that ball_box is valid
            print(f"Error: No ball box for frame {frame_num}")
            output_player_boxes.append({})
            continue
